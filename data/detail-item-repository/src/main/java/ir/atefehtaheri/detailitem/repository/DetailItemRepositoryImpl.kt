...
import ir.atefehtaheri.detailitem.repository.models.asBackdropsDataModel
...
class DetailItemRepositoryImpl @Inject constructor(
    ...
    override suspend fun getBackdropsItem(
        id: String,
        type: Type
    ): ResultStatus<backdropsDataModel> {
        return when (val result = DetailItemDatasource.getBackdropsItem(id,type)) {
            is ResultStatus.Failure -> ResultStatus.Failure(result.exception_message)
            is ResultStatus.Success -> ResultStatus.Success(result.data?.asBackdropsDataModel())
        }
    }
}